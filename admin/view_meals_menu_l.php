	...
	<div class="row"> <!-- Row 1 starts -->
		<div class="col-lg-12"> <!-- To make website responsive -->
			<ol class="breadcrumb"> <!-- To create horizontal navigation bar -->
				<li class="active">
					<i class="fa fa-dashboard"></i> Dashboard / Views Meals Menu for Lunch
				</li>
			</ol>
		</div>
	</div>
			...
			<div class="panel panel-default"> <!-- To colour the style of the panel  -->
				<div class="panel-heading"> <!-- To create a padding box around the heading of the coupon panel -->
					<h3 class="panel-title">
						<i class="fa fa-edit"></i> Meals Menu for Lunch
					</h3>
				</div>	
							...
							<!-- To create table carrier -->
								...
								<table class="table table-bordered table-hover table-striped"> <!-- To create table striped -->
									<thead><!-- thead Starts -->
										...
										$i= $start_from;
										$get_meals_menu_l = "select * from meals_menu_l order by meals_menu_l_id desc limit $start_from, $num_per_page";
										$run_meals_menu_l = mysqli_query($con,$get_meals_menu_l);
										while($row_meals_menu_l = mysqli_fetch_array($run_meals_menu_l)){
											
											$meals_portion_id=$row_meals_menu_l["meals_portion_id"];
										...
										<tr>
											...
											<td> <?php echo $row_meals_menu_l["meals_menu_l"] ?></td>
											<td> <?php echo $row_meals_menu_l["meals_quantity_l"] ?> gram</td>
											<td> RM <?php echo $row_meals_menu_l["meals_cost"] ?></td>

											<td> 
											<?php $meals_menu_l_id=$row_meals_menu_l["meals_menu_l_id"] ?>
												<a href="index.php?edit_meals_menu_l=<?php echo $meals_menu_l_id; ?>">
											...
											<td> 
											<?php $meals_menu_l_id=$row_meals_menu_l["meals_menu_l_id"] ?>
												<a href="index.php?delete_meals_menu_l=<?php echo $meals_menu_l_id; ?>">
									...
									<?php
										$get_page = "select * from meals_menu_l";
										$run_page = mysqli_query($con,$get_page);
										$total_record = mysqli_num_rows($run_page);
										$total_page = ceil($total_record/$num_per_page);

										if ($page>1){
											echo "<a href='index.php?view_meals_menu_l=".($page - 1)."'class='btn btn-danger'>Previous</a>";
										}
										
										for ($i=1;$i<$total_page;$i++){
											echo "<a href='index.php?view_meals_menu_l=".$i."'class='btn btn-primary'>$i</a>";
										}

										if ($i>$page){
											echo "<a href='index.php?view_meals_menu_l=".($page + 1)."'class='btn btn-danger'>Next</a>";
										}
